...
export function ActionBar({ editing, hasChanges, canWrite, writeReason, writeWarning, busy, onStartEdit, onCancelEdit, onSave }: ActionBarProps) {
  if (!editing) {
    return (
      <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canWrite && writeWarning && (
          <p className="flex items-center gap-1.5 text-xs text-[var(--dt-danger)]">
            <AlertIcon width={14} height={14} />
            {writeWarning}
          </p>
        )}
